 ...
 */
async function ensureAnimeEmployees(){
...
export async function render() {
  ...
  app.innerHTML = `
    <div class="flex items-center justify-between mb-4">
      ...
      <div class="flex items-center gap-2">
        <button id="btn-archived-employees" class="inline-flex items-center px-3 py-2 text-sm font-medium rounded border border-gray-300 text-gray-700 hover:bg-gray-50" title="Archived Employees">
          <i class="fa-solid fa-box-archive mr-2"></i>
          <span>Archived</span>
          <span id="archived-count-badge" class="ml-2 hidden text-xs rounded-full bg-gray-200 px-2 py-0.5"></span>
        </button>
    ...
    <!-- Archived Employees modal (front-end only) -->
    <div id="archiveEmployeesModal" class="fixed inset-0 z-50 hidden">
      <div class="absolute inset-0 bg-black/50" data-close="true"></div>
      <div class="relative mx-auto mt-20 w-full max-w-2xl">
        <div class="bg-white rounded-lg shadow">
          <div class="flex items-center justify-between border-b px-4 py-3">
            <h5 class="font-semibold">Archive Employees</h5>
            <button class="text-gray-500 hover:text-gray-700 text-xl leading-none" data-close="true">×</button>
          </div>
          <div class="p-4">
            <div class="mb-3 flex items-center gap-3">
              <div class="relative flex-1">
                <svg class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-4.35-4.35M10 18a8 8 0 100-16 8 8 0 000 16z"/></svg>
                <input id="archived-search" class="w-full border rounded pl-9 pr-3 py-1.5 text-sm" placeholder="Search archived items" />
              </div>
              <div>
                <select id="archived-dept-filter" class="border rounded px-2 py-1 text-sm">
                  <option value="">All departments</option>
                </select>
              </div>
            </div>
            <div id="archived-list" class="text-sm text-gray-700">
              <div class="text-gray-500">No archived items.</div>
            </div>
          </div>
          <div class="flex items-center justify-between border-t px-4 py-3">
            <div class="text-sm text-gray-500"><span id="archived-total">0</span> item(s) archived</div>
            <div class="flex gap-2">
              <button id="archived-clear" class="px-3 py-2 text-sm rounded border">Clear</button>
              <button id="archived-restore-all" class="px-3 py-2 text-sm rounded bg-primary-600 text-white hover:bg-primary-700">Restore All</button>
            </div>
          </div>
        </div>
      </div>
  ...
  // Archive helpers (database-backed)
  async function fetchArchivedEmployees(){
    try {
      const res = await axios.get(`${window.baseApiUrl}/employees.php`, { params: { operation: 'listArchivedEmployees' } });
      const rows = Array.isArray(res.data) ? res.data : [];
      return rows.map(r => ({
        archive_id: r.archive_id,
        employee_id: r.employee_id,
        name: toTitleCase(`${r.first_name || ''} ${r.last_name || ''}`.trim()),
  ...
  async function updateArchivedBadge(){
    try {
      const list = await fetchArchivedEmployees();
      const b = document.getElementById('archived-count-badge');
      if (b) { b.textContent = String(list.length); b.classList.toggle('hidden', list.length <= 0); }
    } catch {}
  }
  // Archived modal wiring and actions (DB)
  const archivedBtn = document.getElementById('btn-archived-employees');
  const archivedModal = document.getElementById('archiveEmployeesModal');
  // Hide archive icon in Manager portal (admin-only feature)
  if (isManagerPortal() && archivedBtn) archivedBtn.classList.add('hidden');
  function closeArchiveModal(){ if (archivedModal) archivedModal.classList.add('hidden'); }
  let __archivedListCache = [];
  let archivedDeptFilter = '';
  let archivedSelected = new Set();
  async function renderArchivedList(query){
    const q = String(query || '').toLowerCase();
    const listWrap = document.getElementById('archived-list');
    const totalEl = document.getElementById('archived-total');
    if (!Array.isArray(__archivedListCache) || !__archivedListCache.length) {
      __archivedListCache = await fetchArchivedEmployees();
    }
    let items = __archivedListCache.filter(a => {
      const name = String(a.name || '').toLowerCase();
      const dept = String(a.department || '').toLowerCase();
      const code = formatEmployeeCode(a.employee_id || 0).toLowerCase();
      return !q || name.includes(q) || dept.includes(q) || code.includes(q);
    });
    // Apply department filter if selected
    if (archivedDeptFilter) {
      const deptFilter = String(archivedDeptFilter || '').toLowerCase();
      items = items.filter(a => String(a.department || '').toLowerCase() === deptFilter);
    }
    if (totalEl) totalEl.textContent = String(__archivedListCache.length || 0);
    if (!listWrap) return;
    if (!items.length) { listWrap.innerHTML = '<div class="text-gray-500">No archived items.</div>'; return; }
    const rows = items.map(a => {
      const code = formatEmployeeCode(a.employee_id || 0);
      const when = a.archived_at ? new Date(a.archived_at).toLocaleString() : '';
      const isChecked = archivedSelected.has(String(a.archive_id || ''));
      return '<div class="flex items-center justify-between border-b last:border-0 py-2">'
        + '<div class="flex items-center gap-2">'
        + '<input type="checkbox" class="archived-select h-4 w-4" data-arch-id="' + escapeHtml(String(a.archive_id || '')) + '" data-emp-id="' + escapeHtml(String(a.employee_id || '')) + '" ' + (isChecked ? 'checked' : '') + '/>'
        + '<div>'
        + '<div class="font-medium">' + escapeHtml(a.name || '') + '</div>'
        + '<div class="text-xs text-gray-500">' + escapeHtml(code) + (a.department ? ' • ' + escapeHtml(toTitleCase(a.department)) : '') + '</div>'
        + '</div>'
        + '</div>'
    ...
    // Wire checkbox persistence for archived selection
    try {
      const boxes = listWrap.querySelectorAll('input.archived-select[data-arch-id]');
      boxes.forEach(cb => {
        const id = cb.getAttribute('data-arch-id') || '';
        if (archivedSelected.has(id)) cb.checked = true;
        cb.addEventListener('change', () => {
          if (cb.checked) archivedSelected.add(id);
          else archivedSelected.delete(id);
        });
      });
    } catch {}
  }
  async function openArchiveModal(){
    ...
    const depSel = document.getElementById('archived-dept-filter');
    if (depSel) {
      const opts = Array.isArray(departmentOptions) ? departmentOptions.slice() : [];
      const prev = archivedDeptFilter || '';
      depSel.innerHTML = '<option value="">All departments</option>' + opts.map(d => `<option value="${escapeHtml(d)}">${escapeHtml(d)}</option>`).join('');
      if (prev) {
        const match = opts.find(x => String(x).toLowerCase() === String(prev).toLowerCase());
        depSel.value = match || '';
      }
    }
  ...
  if (archivedBtn) archivedBtn.addEventListener('click', () => { openArchiveModal(); });
  if (archivedModal) {
    archivedModal.querySelectorAll('[data-close="true"]').forEach(el => el.addEventListener('click', closeArchiveModal));
  }
  const archSearch = document.getElementById('archived-search');
  if (archSearch) archSearch.addEventListener('input', () => renderArchivedList(archSearch.value || ''));
  const archDeptSel = document.getElementById('archived-dept-filter');
  if (archDeptSel) archDeptSel.addEventListener('change', () => {
    archivedDeptFilter = archDeptSel.value || '';
    renderArchivedList(archSearch ? (archSearch.value || '') : '');
  });
  const archClear = document.getElementById('archived-clear');
  if (archClear) archClear.addEventListener('click', async () => {
    try {
      await axios.post(`${window.baseApiUrl}/employees.php`, buildFormData({ operation: 'clearEmployeeArchive' }));
      __archivedListCache = [];
      await renderArchivedList('');
      await loadEmployees();
      ensureToastContainer(); showToast('Archive cleared', 'success');
      await updateArchivedBadge();
    } catch { ensureToastContainer(); showToast('Failed to clear archive', 'error'); }
  });
  const archRestoreAll = document.getElementById('archived-restore-all');
  if (archRestoreAll) archRestoreAll.addEventListener('click', async () => {
    try {
      await axios.post(`${window.baseApiUrl}/employees.php`, buildFormData({ operation: 'restoreAllArchivedEmployees' }));
      __archivedListCache = [];
      await renderArchivedList('');
      await loadEmployees();
      ensureToastContainer(); showToast('All archived employees restored', 'success');
      await updateArchivedBadge();
    } catch { ensureToastContainer(); showToast('Failed to restore', 'error'); }
  });
  updateArchivedBadge();
  ...
  await loadDepartmentOptions();
  await loadEmployees();
  // Manager portal: animate stats cards using anime.js
  if (isManagerPortal()) {
    try {
      const anime = await ensureAnimeEmployees();
      if (anime) {
        const statsWrap = document.getElementById('employees-stats');
        const cards = statsWrap ? Array.from(statsWrap.children) : [];
        if (cards.length) {
          cards.forEach(c => { c.style.transformOrigin = '50% 50%'; c.style.opacity = '0'; });
          anime({
            targets: cards,
            translateY: [6, 0],
            scale: [0.98, 1],
  ...
  function renderEmployeesTable() {
        ...
        <td class="px-3 py-2 text-sm text-right">
          <div class="relative inline-block text-left" data-emp-menu-container>
            ...
            <div class="hidden origin-top-right absolute right-0 mt-2 w-36 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 z-10" role="menu" data-menu>
              <div class="py-1">
                <button class="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 inline-flex items-center gap-2" data-menu-action="preview" role="menuitem">
                ...
                <button class="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 inline-flex items-center gap-2" data-menu-action="edit" role="menuitem">
                  <svg class="w-4 h-4 text-blue-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536M4 20h4l10.606-10.606a2 2 0 10-2.828-2.828L5.172 17.172A4 4 0 004 20z" />
                  </svg>
                  <span>Edit</span>
                </button>
                <button class="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 inline-flex items-center gap-2" data-menu-action="archive" role="menuitem">
                  <svg class="w-4 h-4 text-gray-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V7a2 2 0 00-2-2h-3l-1-1h-4l-1 1H6a2 2 0 00-2 2v6m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0H4" />
                  </svg>
                  <span>Archive</span>
                </button>
                <button class="w-full text-left px-3 py-2 text-sm text-red-600 hover:bg-gray-50 inline-flex items-center gap-2" data-menu-action="delete" role="menuitem">
                  <svg class="w-4 h-4 text-red-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-1 12a2 2 0 01-2 2H8a2 2 0 01-2-2L5 7m5-3h4m-7 3h10" />
                  </svg>
                  <span>Delete</span>
                </button>
              </div>
        ...
        if (onArchive) onArchive.addEventListener('click', async (ev) => {
          ...
          try {
            ...
            if (result.isConfirmed) {
              const res = await axios.post(`${window.baseApiUrl}/employees.php`, buildFormData({ operation: 'archiveEmployee', json: JSON.stringify({ employee_id: e.employee_id }) }));
              const ok = res && res.data && (res.data.success === 1 || res.data === 1 || String(res.data) === '1' || (res.data && res.data.success));
              if (ok) {
                await loadEmployees();
                await updateArchivedBadge();
                await Swal.fire({ title: 'Archived!', text: 'Employee has been archived.', icon: 'success' });
              } else {
                await Swal.fire({ title: 'Error', text: 'Failed to archive employee.', icon: 'error' });
              }
            }
          } catch {
            ensureToastContainer(); showToast('Failed to archive employee', 'error');
          }
        });
    ...
    // Manager portal: animate employees table rows using anime.js
    if (isManagerPortal()) {
      try {
        ensureAnimeEmployees().then(anime => {
          if (!anime) return;
          const rows = Array.from(container.querySelectorAll('tbody tr'));
          if (!rows.length) return;
          rows.forEach(r => { r.style.transformOrigin = '50% 50%'; r.style.opacity = '0'; });
          anime({
            targets: rows,
            translateY: [6, 0],
            scale: [0.99, 1],
