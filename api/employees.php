<?php
  ...
  switch ($operation) {
    ...
    case 'archiveEmployee':
      echo archiveEmployee($conn, $json);
      break;
    case 'listArchivedEmployees':
      echo listArchivedEmployees($conn);
      break;
    case 'restoreAllArchivedEmployees':
      echo restoreAllArchivedEmployees($conn);
      break;
    case 'clearEmployeeArchive':
      echo clearEmployeeArchive($conn);
      break;
    default:
      echo json_encode([]);
  }
  ...
  function getEmployees($conn){
    $includeArchived = isset($_GET['include_archived']) ? intval($_GET['include_archived']) : 0;
    $where = $includeArchived ? '' : ' WHERE NOT EXISTS (SELECT 1 FROM tblemployee_archive a WHERE a.employee_id = e.employee_id)';
    $sql = "SELECT e.*, f.bank_account, f.tax_id, f.sss_number, f.philhealth_number, f.pagibig_number, u.role AS user_role
            ...
            LEFT JOIN tblusers u ON u.employee_id = e.employee_id" . $where . "
...
/**
 * Archive employee into tblemployee_archive and mark status inactive.
 */
function archiveEmployee($conn, $json){
  try {
    $data = json_decode($json, true);
    $empId = isset($data["employee_id"]) ? intval($data["employee_id"]) : 0;
    $reason = isset($data["reason"]) ? trim($data["reason"]) : null;
    if ($empId <= 0) return json_encode(["success" => 0, "message" => "Invalid employee"]);
    $archivedBy = isset($_SESSION['user_id']) ? intval($_SESSION['user_id']) : null;
    $conn->beginTransaction();
    $ins = $conn->prepare("INSERT INTO tblemployee_archive (employee_id, first_name, middle_name, last_name, date_of_birth, gender, email, phone, profile_image, address, department, position, basic_salary, salary_rate_type, date_hired, status, created_at, archived_reason, archived_by)
      SELECT e.employee_id, e.first_name, e.middle_name, e.last_name, e.date_of_birth, e.gender, e.email, e.phone, e.profile_image, e.address, e.department, e.position, e.basic_salary, e.salary_rate_type, e.date_hired, e.status, e.created_at, :reason, :archived_by
      FROM tblemployees e WHERE e.employee_id = :id");
    if ($reason === null || $reason === '') { $ins->bindValue(':reason', null, PDO::PARAM_NULL); } else { $ins->bindParam(':reason', $reason); }
    if ($archivedBy === null) { $ins->bindValue(':archived_by', null, PDO::PARAM_NULL); } else { $ins->bindParam(':archived_by', $archivedBy, PDO::PARAM_INT); }
    $ins->bindParam(':id', $empId, PDO::PARAM_INT);
    $ins->execute();
    $upd = $conn->prepare("UPDATE tblemployees SET status = 'inactive' WHERE employee_id = :id");
    $upd->bindParam(':id', $empId, PDO::PARAM_INT);
    $upd->execute();
    $conn->commit();
    return json_encode(["success" => 1]);
  } catch (Exception $e) {
    try { $conn->rollBack(); } catch (Exception $x) {}
    return json_encode(["success" => 0, "message" => "Archive failed"]);
  }
}

/**
 * List archived employees from tblemployee_archive.
 */
function listArchivedEmployees($conn){
  try {
    $sql = "SELECT archive_id, employee_id, first_name, last_name, department, archived_reason, archived_by, archived_at FROM tblemployee_archive ORDER BY archived_at DESC";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    return json_encode($rows ?: []);
...
/**
 * Restore all archived employees: mark active and clear archive.
 */
function restoreAllArchivedEmployees($conn){
  try {
    $conn->beginTransaction();
    $conn->exec("UPDATE tblemployees SET status = 'active' WHERE employee_id IN (SELECT employee_id FROM tblemployee_archive)");
    $del = $conn->prepare("DELETE FROM tblemployee_archive");
    $del->execute();
    $conn->commit();
    return json_encode(["success" => 1]);
  } catch (Exception $e) {
...
/**
 * Clear archived employees without restoring them.
 */
function clearEmployeeArchive($conn){
  try {
    $del = $conn->prepare("DELETE FROM tblemployee_archive");
    $del->execute();
    return json_encode(["success" => 1]);
