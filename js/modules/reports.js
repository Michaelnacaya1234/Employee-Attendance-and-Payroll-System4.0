...
.            <img src="../../images/unitop.png" onerror="this.style.display='none'" />
  ...
  // Ensure Generate and Export handlers are wired (idempotent)
  const __gen = document.getElementById('btn-generate');
  if (__gen && !__gen.__wired) {
    __gen.__wired = true;
    __gen.addEventListener('click', async (e) => {
      e.preventDefault();
      try {
        __gen.disabled = true;
        __gen.textContent = 'Generating...';
        const filters = getFiltersFromUI();
        await generateReport(activeTab, filters);
        page = 1;
        await loadList();
      } catch {
        await safeToast('Failed to generate report', 'error');
      } finally {
        __gen.disabled = false;
        __gen.textContent = 'Generate Report';
      }
    });
  }

  const __expX = document.getElementById('btn-export-excel');
  if (__expX && !__expX.__wired) {
    __expX.__wired = true;
    __expX.addEventListener('click', async (e) => { e.preventDefault(); await exportReportsExcel(activeTab, getFiltersFromUI()); });
  }
  const __expP = document.getElementById('btn-export-pdf');
  if (__expP && !__expP.__wired) {
    __expP.__wired = true;
    __expP.addEventListener('click', async (e) => { e.preventDefault(); await exportReportsPdf(activeTab); });
  }
  const __print = document.getElementById('btn-print');
  if (__print && !__print.__wired) {
    __print.__wired = true;
    __print.addEventListener('click', (e) => { e.preventDefault(); printTable(); });
  }
}
